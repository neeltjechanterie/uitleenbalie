...
echo '<li role="presentation" class="divider"></li><li role="presentation" class="dropdown-header">'.$type.'</li>';
 ...
 // Display title of each markup samples as a select detail
 function listElementsAsDetail ($type) {
     $files = array();
     $handle=opendir($type.'/');
     while (false !== ($file = readdir($handle))):
         if(stristr($file,'.html')):
  ...
  function showMarkup($type) {
    ...
    foreach ($files as $file):
        ...
        echo '<div class="sg-markup sg-section" id="sg-'.$filename.'">';
        echo '<div class="sg-display">';
        echo '<h2 class="sg-h2">'.$title.'</h2>';
...
//          echo '<div class="col-md-5"><div class="well sg-doc">';
